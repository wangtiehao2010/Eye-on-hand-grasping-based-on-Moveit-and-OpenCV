#include "panda_grasping/vision_manager.h"
...
void VisionManager::get2DLocation(cv::Mat img, float &x, float &y)
{
	...
	cv::Rect tablePos; //cv::Rect 等于 cv::Rect2i 等于 Rect_<int>, 内含2d长方体的长度height 宽度width 以及左顶点的x y坐标系
...
void VisionManager::detectTable(cv::Rect &tablePos)
{
    ...
    //splits a multi-channel array into three single-channel arrays
	split(image, BGR);
	cv::Mat gray_image_red = BGR[2];
	cv::Mat gray_image_green = BGR[1];
	cv::Mat denoiseImage;
	cv::medianBlur(gray_image_red, denoiseImage, 3); //中位模糊
	...
	for (int i = 0; i < binaryImage.rows; i++)
	{
		...
		{
			int editValue = binaryImage.at<uchar>(i, j); //uchar 取值范围为0-255，符合灰度范围要求
	...
	cv::circle(image, pt, 2, cv::Scalar(0, 0, 255), -1, 8); //thickness取-1代表填满整个圆
